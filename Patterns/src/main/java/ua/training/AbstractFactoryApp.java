...
class UaMouse implements Mouse {
    ...
    public void scroll(int direction) {
        if(direction > 0) {
            System.out.println("Прокрутити вгору");
        } else  if(direction < 0) {
            System.out.println("Прокрутити вниз");
        } else {
            System.out.println("Не прокручуємо");
        }
    }
}
...
class UaTouchpad implements Touchpad {
    ...
    public void track(int deltaX, int deltaY) {
        int s = (int) Math.sqrt(Math.pow(deltaX, 2) +
                Math.pow(deltaY, 2));
        System.out.println("Перемістились на " + s + " пікселя");
    }
}
